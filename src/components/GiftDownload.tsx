...
import { downloadPrintKit, type PrintKitData } from "@/lib/printkit";
...
const KIT_FILES = [
  { label: "The Letter", note: "A4 · fold guides" },
  { label: "The Envelope", note: "A4 · cut & fold net" },
  { label: "The Postcard", note: "4×6in · 2 sides" },
  { label: "How To Make It", note: "A6 instruction card" },
  { label: "Wax Seals", note: "A4 · 6 stickers" },
];

export function GiftDownload({ data, giftUrl, recipient }: { data: PrintKitData; giftUrl: string; recipient: string }) {
    ...
    <div className="mx-auto w-full max-w-2xl px-6 py-16 text-center" style={{ color: "#ece3d6" }}>
      ...
      <section className="mt-6 rounded-2xl p-6 text-left" style={{ border: "1px solid rgba(243,201,105,0.28)", background: "rgba(243,201,105,0.05)" }}>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">
          {KIT_FILES.map((f) => (
            <div key={f.label} className="rounded-lg p-3 text-center" style={{ border: "1px solid rgba(243,201,105,0.22)" }}>
      ...
      <p className="mx-auto mt-12 max-w-md text-[9px] leading-relaxed tracking-[0.22em] uppercase" style={{ color: "#7f8aa6" }}>
        Sky We Share ✦ Built in love, for someone who gazed at the moon and never knew how much she was watched over by it.
      </p>
    </div>
  );
}
